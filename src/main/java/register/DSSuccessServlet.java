...
@WebServlet("/register/DSSuccessServlet")
public class DSSuccessServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private CustomerDao customerDao;
	public void init() {
        customerDao = new CustomerDao();
    }
    public DSSuccessServlet() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if (request.getSession().getAttribute("loggedIn") != null && (boolean) request.getSession().getAttribute("loggedIn")) {
            ...
            RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/views/depositsuccess.jsp");
